...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional glob pattern for the nominal mSD selection directory. "
            "If provided, this overrides --pt-pattern for the nominal extraction."
        ),
    )
    parser.add_argument(
        "--msd-alternatives",
        default="",
        help=(
            "Comma-separated list of glob patterns for alternative mSD selection directories. "
            "If provided, the mSD systematic is computed as max(|SF_alt - SF_nom|) at "
            "the nominal tau21 working point."
        ),
    )
    parser.add_argument(
        "--tau21-nominal",
    ...
    # values_by_poi[poi][year][tau21] = POIResult for nominal mSD dir
    values_by_poi: Dict[str, Dict[str, Dict[str, POIResult]]] = {p: {} for p in pois}
    # msd_alt_values[poi][year][msd_label] = POIResult (nominal tau21 only)
    msd_alt_values: Dict[str, Dict[str, Dict[str, POIResult]]] = {p: {} for p in pois}
    ...
    msd_nominal_pattern = args.msd_nominal_pattern or args.pt_pattern
    msd_alt_patterns = [p.strip() for p in args.msd_alternatives.split(",") if p.strip()]
    ...
    for year in years:
        ...
        # Nominal mSD directory (also used for tau21 variation)
        nominal_dir = _find_unique_dir(year_dir, msd_nominal_pattern)
        tau_dirs = sorted(
            [
                p
                for p in glob.glob(os.path.join(nominal_dir, "tau21_*"))
                if os.path.isdir(p)
            ]
        )
        if len(tau_dirs) == 0:
            raise FileNotFoundError(
                f"No tau21_* subdirectories found under: {nominal_dir}"
            )
        ...
        if missing:
            raise FileNotFoundError(
                f"Missing required tau21 folders under: {nominal_dir}\n"
                f"Missing: {missing}\n"
                f"Found: {sorted(found_tau21)}"
            )

        # Optional mSD variations (use nominal tau21 only)
        for pattern in msd_alt_patterns:
            alt_dir = _find_unique_dir(year_dir, pattern)
            tau_dir = os.path.join(alt_dir, f"tau21_{tau21_nominal}")
            if not os.path.isdir(tau_dir):
                raise FileNotFoundError(
                    f"Missing nominal tau21 folder under: {alt_dir}\n"
                    f"Expected: {tau_dir}"
                )
            fit_file = _find_fit_file(tau_dir)
            alt_label = os.path.basename(alt_dir)

            tf, fit_s = _load_fit_s(fit_file)
            try:
                for poi in pois:
                    msd_alt_values.setdefault(poi, {}).setdefault(year, {})[alt_label] = _get_poi_result(
                        fit_s, poi
                    )
            finally:
                tf.Close()
    ...
    # Clean pt pattern for file name (remove wildcards and non-alphanum except dash/underscore)
    import re
    pt_pattern_clean = re.sub(r'[^\w\-]', '', args.pt_pattern.replace('*', ''))
    if pt_pattern_clean:
        pt_pattern_str = f"_{pt_pattern_clean}"
    else:
        pt_pattern_str = ""
    ...
    for year in years:
        ...
        out_path = os.path.join(args.output_dir, f"{stem}{pt_pattern_str}_{year}{ext}")
        cset = {
            "schema_version": 2,
            "description": f"{args.description} | {year}",
